// 空间级别枚举常量（对应后端 Java SpaceLevelEnum 枚举）
export const SPACE_LEVEL_ENUM = {
  COMMON: 0,
  PROFESSIONAL: 1,
  FLAGSHIP: 2,
} as const

// 空间级别数字value -> 中文文本映射（用于表格回显、文字展示）
export const SPACE_LEVEL_MAP: Record<number, string> = {
  0: '普通版',
  1: '专业版',
  2: '旗舰版',
}

// 空间级别下拉选择框 Options（适配 Ant Design Vue Select 组件）
export const SPACE_LEVEL_OPTIONS: Array<{ label: string; value: number }> = Object.keys(
  SPACE_LEVEL_MAP,
).map((key: string) => {
  // 把Object遍历出来的字符串key 转为数字类型（后端存的是数字int）
  const value: number = Number(key)
  return {
    label: SPACE_LEVEL_MAP[value],
    value,
  }
})

// 空间类型枚举
export const SPACE_TYPE_ENUM = {
  PRIVATE: 0,
  TEAM: 1,
}

// 空间类型文本映射
export const SPACE_TYPE_MAP: Record<number, string> = {
  0: '私有空间',
  1: '团队空间',
}

// 空间类型选项映射
export const SPACE_TYPE_OPTIONS = Object.keys(SPACE_TYPE_MAP).map((key) => {
  const value = Number(key) // 将字符串 key 转换为数字
  return {
    label: SPACE_TYPE_MAP[value],
    value,
  }
})

// 空间角色枚举（补充 CREATOR 角色）
export const SPACE_ROLE_ENUM = {
  VIEWER: 'viewer',
  EDITOR: 'editor',
  ADMIN: 'admin',
  CREATOR: 'creator', // 新增创建者枚举
} as const

// 空间角色文本映射（补充 creator 对应文本）
export const SPACE_ROLE_MAP: Record<string, string> = {
  viewer: '浏览者',
  editor: '编辑者',
  admin: '管理员',
  creator: '创建者', // 新增创建者文本映射
}

// 空间角色选项映射（自动包含 creator，无需额外修改遍历逻辑）
export const SPACE_ROLE_OPTIONS = Object.keys(SPACE_ROLE_MAP).map((key) => {
  return {
    label: SPACE_ROLE_MAP[key],
    value: key,
  }
})

/**
 * 空间权限常量（补充解散空间权限）
 */
export const SPACE_PERMISSION_ENUM = {
  SPACE_USER_MANAGE: 'spaceUser:manage',
  PICTURE_VIEW: 'picture:view',
  PICTURE_UPLOAD: 'picture:upload',
  PICTURE_EDIT: 'picture:edit',
  PICTURE_DELETE: 'picture:delete',
  DISSOLVE: 'dissolve', // 新增解散空间权限常量
} as const
